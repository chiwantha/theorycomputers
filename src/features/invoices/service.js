...
export const createInvoice = async (body) => {
  ...
  try {
    ...
    if (invItems.length > 0) {
      validateInvItems(invItems, docType);
    }
    ...
    if (!resInvHeader.insertId) {
      throw new AppError("Invoice Header Failed !", 500);
    }
    ...
    if (payment_rows.length > 0) {
      for (const row of payment_rows) {
        ...
        if (!resPayments.insertId) {
          throw new AppError(`Failed to Save Payments !`, 500);
        }
      }
    }
    ...
    if (invItems.length > 0) {
      for (const item of invItems) {
        ...
        if (!resInvItems.insertId) {
          throw new AppError(
            `Failed to Save Invoice Item ${item?.itemName} !`,
            500,
          );
        }
      ...
      throw new AppError(`Failed to Find Items !`, 404);
    }
    ...
    if (invType === `JOB`) {
      ...
      if (resJobStatus.affectedRows === 0) {
        throw new AppError(`Update Job Payment Failed !`, 500);
      }
    }
